SET ANSI_NULLS ON;
GO
SET QUOTED_IDENTIFIER ON;
GO

/* ===== [dbo].[tr_Audit_BongkarSusunInputBonggolan] ON [dbo].[BongkarSusunInputBonggolan] =====
   AFTER INSERT, UPDATE, DELETE
   Action: CONSUME_FULL / UNCONSUME_FULL / UPDATE
   GROUPING: 1 row audit per (NoBongkarSusun, NoBonggolan) per statement
   PK JSON: { NoBongkarSusun, NoBonggolan }
   NewData/OldData: array of {NoBongkarSusun, NoBonggolan, Berat}
   NOTE:
   - Table hanya punya 2 kolom, jadi untuk "Berat" kita enrichment dari tabel master Bonggolan.
     Jika beratnya bukan di dbo.Bonggolan, ganti join + field sesuai tabelmu.
*/
CREATE OR ALTER TRIGGER [dbo].[tr_Audit_BongkarSusunInputBonggolan]
ON [dbo].[BongkarSusunInputBonggolan]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
  SET NOCOUNT ON;

  DECLARE @actor nvarchar(128) =
    COALESCE(
      CONVERT(nvarchar(128), TRY_CONVERT(int, SESSION_CONTEXT(N'actor_id'))),
      CAST(SESSION_CONTEXT(N'actor') AS nvarchar(128)),
      SUSER_SNAME()
    );

  DECLARE @rid nvarchar(64) =
    CAST(SESSION_CONTEXT(N'request_id') AS nvarchar(64));

  /* =========================================================
     1) INSERT-only rows => CONSUME_FULL (GROUPED)
     ========================================================= */
  ;WITH insOnly AS (
    SELECT i.*
    FROM inserted i
    LEFT JOIN deleted d
      ON d.NoBongkarSusun = i.NoBongkarSusun
     AND d.NoBonggolan    = i.NoBonggolan
    WHERE d.NoBongkarSusun IS NULL
  ),
  insEnriched AS (
    SELECT
      x.NoBongkarSusun,
      x.NoBonggolan,
      b.Berat
    FROM insOnly x
    LEFT JOIN dbo.Bonggolan b
      ON b.NoBonggolan = x.NoBonggolan
  ),
  g AS (
    SELECT DISTINCT NoBongkarSusun, NoBonggolan
    FROM insEnriched
  )
  INSERT dbo.AuditTrail(Action, TableName, Actor, RequestId, PK, OldData, NewData)
  SELECT
    'CONSUME_FULL',
    'BongkarSusunInputBonggolan',
    @actor,
    @rid,
    (SELECT gg.NoBongkarSusun, gg.NoBonggolan FOR JSON PATH, WITHOUT_ARRAY_WRAPPER),
    NULL,
    (
      SELECT
        e.NoBongkarSusun,
        e.NoBonggolan,
        CAST(e.Berat AS decimal(18,3)) AS Berat
      FROM insEnriched e
      WHERE e.NoBongkarSusun = gg.NoBongkarSusun
        AND e.NoBonggolan    = gg.NoBonggolan
      FOR JSON PATH
    )
  FROM g gg;

  /* =========================================================
     2) DELETE-only rows => UNCONSUME_FULL (GROUPED)
     ========================================================= */
  ;WITH delOnly AS (
    SELECT d.*
    FROM deleted d
    LEFT JOIN inserted i
      ON i.NoBongkarSusun = d.NoBongkarSusun
     AND i.NoBonggolan    = d.NoBonggolan
    WHERE i.NoBongkarSusun IS NULL
  ),
  delEnriched AS (
    SELECT
      x.NoBongkarSusun,
      x.NoBonggolan,
      b.Berat
    FROM delOnly x
    LEFT JOIN dbo.Bonggolan b
      ON b.NoBonggolan = x.NoBonggolan
  ),
  g AS (
    SELECT DISTINCT NoBongkarSusun, NoBonggolan
    FROM delEnriched
  )
  INSERT dbo.AuditTrail(Action, TableName, Actor, RequestId, PK, OldData, NewData)
  SELECT
    'UNCONSUME_FULL',
    'BongkarSusunInputBonggolan',
    @actor,
    @rid,
    (SELECT gg.NoBongkarSusun, gg.NoBonggolan FOR JSON PATH, WITHOUT_ARRAY_WRAPPER),
    (
      SELECT
        e.NoBongkarSusun,
        e.NoBonggolan,
        CAST(e.Berat AS decimal(18,3)) AS Berat
      FROM delEnriched e
      WHERE e.NoBongkarSusun = gg.NoBongkarSusun
        AND e.NoBonggolan    = gg.NoBonggolan
      FOR JSON PATH
    ),
    NULL
  FROM g gg;

  /* =========================================================
     3) UPDATE rows => UPDATE (GROUPED)
     ========================================================= */
  IF EXISTS (SELECT 1 FROM inserted) AND EXISTS (SELECT 1 FROM deleted)
  BEGIN
    ;WITH upd AS (
      SELECT i.NoBongkarSusun, i.NoBonggolan
      FROM inserted i
      JOIN deleted d
        ON d.NoBongkarSusun = i.NoBongkarSusun
       AND d.NoBonggolan    = i.NoBonggolan
    ),
    g AS (
      SELECT DISTINCT NoBongkarSusun, NoBonggolan
      FROM upd
    )
    INSERT dbo.AuditTrail(Action, TableName, Actor, RequestId, PK, OldData, NewData)
    SELECT
      'UPDATE',
      'BongkarSusunInputBonggolan',
      @actor,
      @rid,
      (SELECT gg.NoBongkarSusun, gg.NoBonggolan FOR JSON PATH, WITHOUT_ARRAY_WRAPPER),

      (
        SELECT
          d.NoBongkarSusun,
          d.NoBonggolan,
          CAST(b.Berat AS decimal(18,3)) AS Berat
        FROM deleted d
        LEFT JOIN dbo.Bonggolan b
          ON b.NoBonggolan = d.NoBonggolan
        WHERE d.NoBongkarSusun = gg.NoBongkarSusun
          AND d.NoBonggolan    = gg.NoBonggolan
        FOR JSON PATH
      ),

      (
        SELECT
          i.NoBongkarSusun,
          i.NoBonggolan,
          CAST(b.Berat AS decimal(18,3)) AS Berat
        FROM inserted i
        LEFT JOIN dbo.Bonggolan b
          ON b.NoBonggolan = i.NoBonggolan
        WHERE i.NoBongkarSusun = gg.NoBongkarSusun
          AND i.NoBonggolan    = gg.NoBonggolan
        FOR JSON PATH
      )
    FROM g gg;
  END
END;
GO
