...
/* ===== [dbo].[tr_Audit_BongkarSusunInputBroker] ON [dbo].[BongkarSusunInputBroker] =====
   AFTER INSERT, UPDATE, DELETE
   Action: CONSUME_FULL / UNCONSUME_FULL / UPDATE
   GROUPING: 1 row audit per (NoBongkarSusun, NoBroker) per statement
   PK JSON: { NoBongkarSusun, NoBroker }
   NewData/OldData: array of {NoBongkarSusun, NoBroker, NoSak, Berat, IsPartial}
*/
CREATE OR ALTER TRIGGER [dbo].[tr_Audit_BongkarSusunInputBroker]
ON [dbo].[BongkarSusunInputBroker]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
  SET NOCOUNT ON;
  ...
  g AS (
    SELECT DISTINCT NoBongkarSusun, NoBroker
    FROM insOnly
  )
  ...
  SELECT
    ...
    @actor,
    @rid,
    (SELECT gg.NoBongkarSusun, gg.NoBroker FOR JSON PATH, WITHOUT_ARRAY_WRAPPER),
    NULL,
    (
      SELECT
        x.NoBongkarSusun,
  ...
  BEGIN
    ;WITH upd AS (
      SELECT i.NoBongkarSusun, i.NoBroker, i.NoSak
      FROM inserted i
      JOIN deleted d
        ON d.NoBongkarSusun = i.NoBongkarSusun
       AND d.NoBroker       = i.NoBroker
