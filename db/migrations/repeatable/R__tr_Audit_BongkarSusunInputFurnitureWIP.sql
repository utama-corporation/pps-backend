...
/* ===== [dbo].[tr_Audit_BongkarSusunInputFurnitureWIP] ON [dbo].[BongkarSusunInputFurnitureWIP] =====
   AFTER INSERT, UPDATE, DELETE
   Action: CONSUME_FULL / UNCONSUME_FULL / UPDATE
   GROUPING: 1 row audit per (NoBongkarSusun, NoFurnitureWIP) per statement
   PK JSON: { NoBongkarSusun, NoFurnitureWIP }
   NewData/OldData: array of {NoBongkarSusun, NoFurnitureWIP, Pcs, Berat, IsPartial}

   Enrichment:
   - Pcs, Berat, IsPartial diambil dari dbo.FurnitureWIP (fw.Pcs, fw.Berat, fw.IsPartial)
*/
CREATE OR ALTER TRIGGER [dbo].[tr_Audit_BongkarSusunInputFurnitureWIP]
ON [dbo].[BongkarSusunInputFurnitureWIP]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
  SET NOCOUNT ON;
  ...
  g AS (
    SELECT DISTINCT NoBongkarSusun, NoFurnitureWIP
    FROM insOnly
  )
  ...
  SELECT
    ...
    @actor,
    @rid,
    (SELECT gg.NoBongkarSusun, gg.NoFurnitureWIP FOR JSON PATH, WITHOUT_ARRAY_WRAPPER),
    NULL,
    (
      SELECT
        x.NoBongkarSusun,
