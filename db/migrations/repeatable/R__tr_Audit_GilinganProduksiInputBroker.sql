/* ===== [dbo].[tr_Audit_GilinganProduksiInputBroker] ON [dbo].[GilinganProduksiInputBroker] =====
   ...
   NewData/OldData: array of {NoProduksi, NoBroker, NoSak, Berat}
   Enrichment:
   - Berat diambil dari dbo.Broker_d (bd.Berat) berdasarkan (NoBroker, NoSak)
*/
CREATE OR ALTER TRIGGER [dbo].[tr_Audit_GilinganProduksiInputBroker]
ON [dbo].[GilinganProduksiInputBroker]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
  SET NOCOUNT ON;
  ...
  SELECT
    ...
    (
      ...
      LEFT JOIN dbo.Broker_d bd
        ON bd.NoBroker = x.NoBroker
       AND bd.NoSak    = x.NoSak
