/* ===== [dbo].[tr_Audit_GilinganProduksiInputCrusher] ON [dbo].[GilinganProduksiInputCrusher] =====
   AFTER INSERT, UPDATE, DELETE
   Action: CONSUME_FULL / UNCONSUME_FULL / UPDATE
   GROUPING: 1 row audit per (NoProduksi, NoCrusher) per statement
   PK JSON: { NoProduksi, NoCrusher }
   NewData/OldData: array of {NoProduksi, NoCrusher, DatetimeInput, Berat}
   Enrichment:
   - Berat diambil dari tabel master Crusher (diasumsikan dbo.Crusher.Berat)
     Kalau tabel/kolom berbeda, ganti join + field di bawah.
*/
CREATE OR ALTER TRIGGER [dbo].[tr_Audit_GilinganProduksiInputCrusher]
ON [dbo].[GilinganProduksiInputCrusher]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
  SET NOCOUNT ON;

  DECLARE @actor nvarchar(128) =
    COALESCE(
      CONVERT(nvarchar(128), TRY_CONVERT(int, SESSION_CONTEXT(N'actor_id'))),
      CAST(SESSION_CONTEXT(N'actor') AS nvarchar(128)),
      SUSER_SNAME()
    );

  DECLARE @rid nvarchar(64) =
    CAST(SESSION_CONTEXT(N'request_id') AS nvarchar(64));

  /* =========================================================
     1) INSERT-only rows => CONSUME_FULL (GROUPED)
     ========================================================= */
  ;WITH insOnly AS (
    SELECT i.*
    FROM inserted i
    LEFT JOIN deleted d
      ON d.NoProduksi = i.NoProduksi
     AND d.NoCrusher  = i.NoCrusher
    WHERE d.NoProduksi IS NULL
  ),
  g AS (
    SELECT DISTINCT NoProduksi, NoCrusher
    FROM insOnly
  )
  INSERT dbo.AuditTrail(Action, TableName, Actor, RequestId, PK, OldData, NewData)
  SELECT
    'CONSUME_FULL',
    'GilinganProduksiInputCrusher',
    @actor,
    @rid,
    (SELECT gg.NoProduksi, gg.NoCrusher FOR JSON PATH, WITHOUT_ARRAY_WRAPPER),
    NULL,
    (
      SELECT
        x.NoProduksi,
        x.NoCrusher,
        x.DatetimeInput,
        CAST(ch.Berat AS decimal(18,3)) AS Berat
      FROM insOnly x
      LEFT JOIN dbo.Crusher ch
        ON ch.NoCrusher = x.NoCrusher
      WHERE x.NoProduksi = gg.NoProduksi
        AND x.NoCrusher  = gg.NoCrusher
      FOR JSON PATH
    )
  FROM g gg;

  /* =========================================================
     2) DELETE-only rows => UNCONSUME_FULL (GROUPED)
     ========================================================= */
  ;WITH delOnly AS (
    SELECT d.*
    FROM deleted d
    LEFT JOIN inserted i
      ON i.NoProduksi = d.NoProduksi
     AND i.NoCrusher  = d.NoCrusher
    WHERE i.NoProduksi IS NULL
  ),
  g AS (
    SELECT DISTINCT NoProduksi, NoCrusher
    FROM delOnly
  )
  INSERT dbo.AuditTrail(Action, TableName, Actor, RequestId, PK, OldData, NewData)
  SELECT
    'UNCONSUME_FULL',
    'GilinganProduksiInputCrusher',
    @actor,
    @rid,
    (SELECT gg.NoProduksi, gg.NoCrusher FOR JSON PATH, WITHOUT_ARRAY_WRAPPER),
    (
      SELECT
        x.NoProduksi,
        x.NoCrusher,
        x.DatetimeInput,
        CAST(ch.Berat AS decimal(18,3)) AS Berat
      FROM delOnly x
      LEFT JOIN dbo.Crusher ch
        ON ch.NoCrusher = x.NoCrusher
      WHERE x.NoProduksi = gg.NoProduksi
        AND x.NoCrusher  = gg.NoCrusher
      FOR JSON PATH
    ),
    NULL
  FROM g gg;

  /* =========================================================
     3) UPDATE rows => UPDATE (GROUPED)
     ========================================================= */
  IF EXISTS (SELECT 1 FROM inserted) AND EXISTS (SELECT 1 FROM deleted)
  BEGIN
    ;WITH upd AS (
      SELECT i.NoProduksi, i.NoCrusher
      FROM inserted i
      JOIN deleted d
        ON d.NoProduksi = i.NoProduksi
       AND d.NoCrusher  = i.NoCrusher
    ),
    g AS (
      SELECT DISTINCT NoProduksi, NoCrusher
      FROM upd
    )
    INSERT dbo.AuditTrail(Action, TableName, Actor, RequestId, PK, OldData, NewData)
    SELECT
      'UPDATE',
      'GilinganProduksiInputCrusher',
      @actor,
      @rid,
      (SELECT gg.NoProduksi, gg.NoCrusher FOR JSON PATH, WITHOUT_ARRAY_WRAPPER),

      (
        SELECT
          d.NoProduksi,
          d.NoCrusher,
          d.DatetimeInput,
          CAST(ch.Berat AS decimal(18,3)) AS Berat
        FROM deleted d
        LEFT JOIN dbo.Crusher ch
          ON ch.NoCrusher = d.NoCrusher
        WHERE d.NoProduksi = gg.NoProduksi
          AND d.NoCrusher  = gg.NoCrusher
        FOR JSON PATH
      ),

      (
        SELECT
          i.NoProduksi,
          i.NoCrusher,
          i.DatetimeInput,
          CAST(ch.Berat AS decimal(18,3)) AS Berat
        FROM inserted i
        LEFT JOIN dbo.Crusher ch
          ON ch.NoCrusher = i.NoCrusher
        WHERE i.NoProduksi = gg.NoProduksi
          AND i.NoCrusher  = gg.NoCrusher
        FOR JSON PATH
      )
    FROM g gg;
  END
END;
GO
