/* ===== [dbo].[tr_Audit_InjectProduksiInputGilinganPartial] ON [dbo].[InjectProduksiInputGilinganPartial] =====
...
*/
CREATE OR ALTER TRIGGER [dbo].[tr_Audit_InjectProduksiInputGilinganPartial]
ON [dbo].[InjectProduksiInputGilinganPartial]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
  SET NOCOUNT ON;
    ...
    SELECT
      ...
      (
        SELECT
          ...
          d.NoGilinganPartial,
          gp2.NoGilingan,
          CAST(gp2.Berat AS decimal(18,3)) AS Berat
        FROM deleted d
        LEFT JOIN dbo.GilinganPartial gp2
          ON gp2.NoGilinganPartial = d.NoGilinganPartial
        WHERE d.NoProduksi        = gg.NoProduksi
          AND d.NoGilinganPartial = gg.NoGilinganPartial
          AND (
                (gp2.NoGilingan = gg.NoGilingan)
                OR (gp2.NoGilingan IS NULL AND gg.NoGilingan IS NULL)
              )
        FOR JSON PATH
      ),
      ...
      (
        ...
        FROM inserted i
        LEFT JOIN dbo.GilinganPartial gp2
          ON gp2.NoGilinganPartial = i.NoGilinganPartial
