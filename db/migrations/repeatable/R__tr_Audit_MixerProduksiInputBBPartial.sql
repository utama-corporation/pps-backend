/* ===== [dbo].[tr_Audit_MixerProduksiInputBBPartial] ON [dbo].[MixerProduksiInputBBPartial] =====
   AFTER INSERT, UPDATE, DELETE
   Action: CONSUME_PARTIAL / UNCONSUME_PARTIAL / UPDATE

   NOTE:
   - AuditTrail 1 row per statement per (NoProduksi, NoBBPartial)
   - PK JSON: NoProduksi, NoBBPartial, NoBahanBaku, NoPallet (tanpa NoSak)
   - Data detail per sak disimpan dalam JSON array (NoSak ikut di array)
   - NoBahanBaku/NoPallet/NoSak/Berat diambil dari dbo.BahanBakuPartial
*/
CREATE OR ALTER TRIGGER [dbo].[tr_Audit_MixerProduksiInputBBPartial]
ON [dbo].[MixerProduksiInputBBPartial]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
  SET NOCOUNT ON;
