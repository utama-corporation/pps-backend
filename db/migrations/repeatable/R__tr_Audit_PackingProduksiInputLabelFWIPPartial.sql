...
/* ===== [dbo].[tr_Audit_PackingProduksiInputLabelFWIPPartial] ON [dbo].[PackingProduksiInputLabelFWIPPartial] =====
   AFTER INSERT, UPDATE, DELETE
   Action: CONSUME_PARTIAL / UNCONSUME_PARTIAL / UPDATE

   NOTE:
   - AuditTrail hanya 1 row per statement per (NoPacking, NoFurnitureWIPPartial, NoFurnitureWIP)
   - PK JSON: {NoPacking, NoFurnitureWIPPartial, NoFurnitureWIP} (TANPA Pcs)
   - OldData/NewData: array of {NoPacking, NoFurnitureWIPPartial, NoFurnitureWIP, Pcs}
   - NoFurnitureWIP + Pcs diambil dari dbo.FurnitureWIPPartial (fwp.NoFurnitureWIP, fwp.Pcs)
*/
CREATE OR ALTER TRIGGER [dbo].[tr_Audit_PackingProduksiInputLabelFWIPPartial]
ON [dbo].[PackingProduksiInputLabelFWIPPartial]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
  SET NOCOUNT ON;
  ...
  /* ======================================================
     ...
     ====================================================== */
  ;WITH insOnly AS (
    SELECT i.NoPacking, i.NoFurnitureWIPPartial
    FROM inserted i
    WHERE NOT EXISTS (
      SELECT 1
      FROM deleted d
      WHERE d.NoPacking            = i.NoPacking
  ...
  grp AS (
    SELECT DISTINCT NoPacking, NoFurnitureWIPPartial, NoFurnitureWIP
    FROM insEnriched
  )
  ...
  SELECT
    ...
    @actor,
    @rid,
    (SELECT g.NoPacking, g.NoFurnitureWIPPartial, g.NoFurnitureWIP FOR JSON PATH, WITHOUT_ARRAY_WRAPPER),
    NULL,
    COALESCE((
      SELECT
        e.NoPacking,
  ...
  /* ======================================================
     ...
     ====================================================== */
  ;WITH delOnly AS (
    SELECT d.NoPacking, d.NoFurnitureWIPPartial
    FROM deleted d
    WHERE NOT EXISTS (
      SELECT 1
      FROM inserted i
      WHERE i.NoPacking            = d.NoPacking
  ...
  BEGIN
    ;WITH grp0 AS (
      SELECT DISTINCT i.NoPacking, i.NoFurnitureWIPPartial
      FROM inserted i
      JOIN deleted d
    ...
    SELECT
      ...
      COALESCE((
        ...
        WHERE d2.NoPacking             = g.NoPacking
      ...
      COALESCE((
        ...
        WHERE i2.NoPacking             = g.NoPacking
