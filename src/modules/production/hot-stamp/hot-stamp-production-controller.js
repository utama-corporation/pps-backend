...
async function completeProduksi(req, res) {
  ...
  try {
    const data = await hotStampingService.completeHotStampingProduksi(
      noProduksi,
      { actorId, actorUsername, requestId },
    );
    return res.status(200).json({ success: true, data });
  } catch (error) {
    console.error("[hotstamp.completeProduksi]", error);
...
async function getStok(req, res) {
  try {
    const data = await hotStampingService.getStok();
    ...
    console.error("Get Stok Hot Stamping Error", err);
    return res
      .status(500)
...
async function getLabelByIdFurnitureWip(req, res) {
  try {
    ...
    const data = await hotStampingService.getLabelByIdFurnitureWIP(
      idFurnitureWip,
    );
