...
async function uncompleteProduksi(req, res) {
  ...
  try {
    const data = await injectProduksiService.uncompleteInjectProduksi(
      noProduksi,
      { actorId, actorUsername, requestId },
    );
    ...
    console.error("[inject.uncompleteProduksi]", error);
    const status = error.statusCode || error.status || 500;
    return res.status(status).json({
      success: false,
      message: status === 500 ? "Internal Server Error" : error.message,
...
async function getStok(req, res) {
  try {
    const data = await injectProduksiService.getStok();
    ...
    console.error('Get Stok Inject Error', err);
    return res
      .status(500)
...
async function getLabelByIdFurnitureWip(req, res) {
  try {
    ...
    const data = await injectProduksiService.getLabelByIdFurnitureWIP(idFurnitureWip);
