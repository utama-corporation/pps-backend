...
async function createProduksi(req, res) {
  try {
    ...
    const payload = {
      ...
      idOperators: Array.isArray(b.idOperators)
...
async function splitProduksiTime(req, res) {
  ...
  try {
    const ctx = { actorId, actorUsername, requestId };
    const result = await keyFittingService.splitProduksiTime(
      { idMesin, tanggal },
    ...
    console.error("[keyFitting.splitProduksiTime]", err);
    const status = err.statusCode || err.status || 500;
    return res.status(status).json({
      success: false,
      message:
...
async function completeProduksi(req, res) {
  ...
  try {
    const data = await keyFittingService.completeKeyFittingProduksi(noProduksi, {
      actorId,
      actorUsername,
      requestId,
    });
    ...
    console.error("[keyfitting.completeProduksi]", error);
...
async function uncompleteProduksi(req, res) {
  ...
  try {
    const data = await keyFittingService.uncompleteKeyFittingProduksi(
      noProduksi,
      { actorId, actorUsername, requestId },
    );
    return res.status(200).json({ success: true, data });
  } catch (error) {
    console.error("[keyfitting.uncompleteProduksi]", error);
    const status = error.statusCode || error.status || 500;
    return res.status(status).json({
      success: false,
      message: status === 500 ? "Internal Server Error" : error.message,
