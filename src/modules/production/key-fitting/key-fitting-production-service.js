...
async function deleteKeyFittingProduksi(noProduksi, ctx) {
  ...
  try {
    ...
    const sqlDelete = `
      ...
      FROM dbo.PasangKunciInputMaterial AS map
      WHERE map.NoProduksi = @NoProduksi;
...
/**
 * Buka kunci produksi: IsComplete 1 -> 0. Kebalikan dari completeKeyFittingProduksi.
 */
async function uncompleteKeyFittingProduksi(noProduksi, ctx) {
  const no = String(noProduksi || "").trim();
