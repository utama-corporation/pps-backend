...
async function completeProduksi(req, res) {
  ...
  try {
    const data = await spannerService.completeSpannerProduksi(noProduksi, {
      actorId,
      actorUsername,
      requestId,
    });
    ...
    console.error("[spanner.completeProduksi]", error);
...
async function uncompleteProduksi(req, res) {
  ...
  try {
    const data = await spannerService.uncompleteSpannerProduksi(noProduksi, {
      actorId,
      actorUsername,
      requestId,
    });
    ...
    console.error("[spanner.uncompleteProduksi]", error);
    const status = error.statusCode || error.status || 500;
    return res.status(status).json({
      success: false,
      message: status === 500 ? "Internal Server Error" : error.message,
