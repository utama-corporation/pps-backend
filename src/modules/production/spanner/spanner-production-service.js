...
async function deleteSpannerProduksi(noProduksi, ctx) {
  ...
  try {
    ...
    const sqlDelete = `
      ...
      FROM dbo.SpannerInputMaterial AS map
      WHERE map.NoProduksi = @NoProduksi;
...
/**
 * Buka kunci produksi: IsComplete 1 -> 0. Kebalikan dari completeSpannerProduksi.
 */
async function uncompleteSpannerProduksi(noProduksi, ctx) {
  const no = String(noProduksi || "").trim();
...
async function completeSpannerProduksi(noProduksi, ctx) {
  const no = String(noProduksi || "").trim();
  if (!no) throw badReq("noProduksi wajib");
...
async function getStok() {
  ...
  const result = await pool.request().query(`
    ...
    EffectiveDetail AS (
      ...
      INNER JOIN SpannerOutputLabelFWIP ip
...
module.exports = {
  ...
  completeSpannerProduksi,
  uncompleteSpannerProduksi,
  fetchInputs,
  fetchOutputs,
  fetchOutputsReject,
