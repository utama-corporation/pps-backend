...
async function completeProduksi(req, res) {
  ...
  try {
    const data = await washingProduksiService.completeWashingProduksi(
      noProduksi,
    ...
    console.error("[washing.completeProduksi]", error);
...
async function uncompleteProduksi(req, res) {
  ...
  try {
    const data = await washingProduksiService.uncompleteWashingProduksi(
      noProduksi,
      { actorId, actorUsername, requestId },
    );
    ...
    console.error("[washing.uncompleteProduksi]", error);
    const status = error.statusCode || error.status || 500;
    return res.status(status).json({
      success: false,
      message: status === 500 ? "Internal Server Error" : error.message,
...
async function getInputsByNoProduksiV2(req, res) {
  ...
  try {
    const data = await washingProduksiService.fetchInputsV2(noProduksi);
    return res
      .status(200)
      .json({ success: true, message: "Inputs retrieved", data });
  } catch (e) {
    console.error("[washing.getInputsByNoProduksiV2]", e);
    return res.status(500).json({
      success: false,
      message: "Internal Server Error",
      error: e.message,
...
async function getOutputsByNoProduksiV2(req, res) {
  ...
  try {
    const data = await washingProduksiService.fetchOutputsV2(noProduksi);
    return res
      .status(200)
      .json({ success: true, message: "Outputs retrieved", data });
  } catch (e) {
    console.error("[washing.getOutputsByNoProduksiV2]", e);
    return res.status(500).json({
      success: false,
      message: "Internal Server Error",
      error: e.message,
...
async function getFormulaInputsByNoProduksi(req, res) {
  ...
  try {
    const result =
      await washingProduksiService.getFormulaInputsByNoProduksi(noProduksi);
    ...
    console.error(
      "Error fetching formula input from WashingProduksi_h:",
      error,
    );
