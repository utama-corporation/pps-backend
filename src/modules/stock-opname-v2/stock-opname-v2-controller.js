...
async function listMyLokasiHandler(req, res) {
  try {
    const data = await stockOpnameV2Service.listMyLokasiWithLabelCount(
      req.idUsername,
    );
    return res.json({
      success: true,
    ...
    console.error("Error fetching my lokasi (lintas NoSO):", error);
    return res.status(error.statusCode || 500).json({
      success: false,
      message: error.message || "Internal Server Error",
      code: error.code,
...
async function listAllUsersHandler(req, res) {
  try {
    const data = await stockOpnameV2Service.listAllUsers();
    return res.json({
      success: true,
...
async function listUsersByLokasiHandler(req, res) {
  ...
  const { stockOpnameNo } = req.query;

  try {
    const data = await stockOpnameV2Service.listUsersByLokasi(
      blok,
      Number(idLokasi),
      stockOpnameNo,
    );
...
async function assignAccessHandler(req, res) {
  ...
  try {
    const result = await stockOpnameV2Service.assignAccess({
      blok,
...
async function revokeAccessHandler(req, res) {
  const { stockOpnameNo, blok, idLokasi, idUsername } = req.params;

  try {
    const result = await stockOpnameV2Service.revokeAccess({
      blok,
      idLokasi: Number(idLokasi),
      idUsername: Number(idUsername),
      stockOpnameNo,
    ...
    return res.json({
      success: true,
      message: `Akses user ${idUsername} ke lokasi ${blok}/${idLokasi} untuk ${stockOpnameNo} berhasil dicabut`,
      data: result,
    });
  } catch (error) {
